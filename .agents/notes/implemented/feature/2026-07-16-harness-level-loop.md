...
Normal turn completion schedules another round only while the goal remains active, armed, and below its cap. Cancellation pauses. Cap exhaustion blocks with `round-limit`; queue failure uses `queue-failed`; policy rejection and unknown terminal results use their corresponding blocker codes. A provider failure (`LlmError`) or a `max-tokens` stop spends a consecutive-failure budget bounded by `maxConsecutiveFailures` (default 3) and is retried, then blocks with code `round-failure`; the goal driver invents no round after any other abnormal terminal outcome. A human can later authorize resume through ordinary language or `/goal resume`.
...
- **Automatically continue after session restore** — rejected because opening a session is observation, not authority to spend resources; that rule survives for a seeded session, where a fork restores the durable state while activation waits for a new human prompt.
...
- **No persistent autonomous runner** — same-session goal facts persist, but activation and scheduling are process-local and deliberately wait for human input after restore into a seeded session; a non-seeded session re-arms its own active goal at the session-start edge. Ralph runs are foreground and cannot resume after process loss. Background collection, restart recovery, and unattended resident execution are deferred.
