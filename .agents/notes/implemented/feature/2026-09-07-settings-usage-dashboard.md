...
UTC dates keep aggregation consistent across browsers. Fork-inherited events are excluded because the source session already owns their usage. Inherited request context still identifies the provider and model for local failed attempts. Each message or failed-attempt event settles one distinct request, including retries that share a step. Valid reported token totals take precedence over component sums. Usage with inconsistent totals or reasoning counts is reported as missing instead of contributing misleading totals; incomplete cache details can still accompany a valid exact total. Missing usage is reported separately instead of estimated.
