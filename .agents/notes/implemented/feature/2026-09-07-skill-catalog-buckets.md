...
Complete JSON pages are checked against a configured UTF-8 byte limit before publication so provider names cannot bypass summary limits. Configuration also bounds the fully framed category message across all categories and count widths before registration. Rejecting oversized pages preserves exact loader names; truncation would produce unusable identifiers. Custom presentation text is complete for initial publication and replacement, and text or revision changes participate in publication identity.
