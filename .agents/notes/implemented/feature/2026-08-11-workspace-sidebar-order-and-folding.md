...
Grouped presentation under Last updated ranks the Workspace sections themselves, not only the rows inside them: each real Workspace sits at its newest visible Session, a Workspace showing no Session follows every Workspace that shows one, equal activity keeps Host order, and Ungrouped stays the trailing bucket. The ranking reads the same visible-Session projection the rows use, so an archived or subagent-origin Session never lifts its Workspace, while the selected provisional New Session does. Manual leaves the sections in Host order.

### Drag and compact chrome

Workspace hit testing uses the complete rendered group section, including visible Session rows. One insertion boundary is shared by the preceding group's lower half and the following group's upper half, and the indicator is an absolutely positioned line with a joined right-facing chevron that does not affect layout. A tree-body overlay draws the first boundary at the same negative offset outside the scrolling clip, so the leading chevron remains visible without moving the list. A collapsed Session drag resolves its insertion boundary from rendered rows, places the source before any hidden account members at that boundary, and rejects a result that would hide the source. A Workspace row carries its drag handle only under Manual order, because Last updated derives the section order and a Host-account move there would resolve against an order the user cannot see. During a Workspace or Session drag, document-level `dragover` and `drop` handlers accept the native operation; if release occurs outside the Workspace list, `dragend` commits the last valid marker.
...
**Keep Workspace sections in durable Host order under Last updated.** Rows would reorder while the section holding them stayed put, so the sidebar's most recent work could sit under a section far down the list — the order the mode is named for would stop at the section boundary.

**Keep the Workspace drag handle under Last updated and let a drop rewrite Host order.** The drop resolves its anchor against Host order while the rows render in activity order, so the committed position would be invisible and unrelated to the gesture.
...
**Persist the expanded-remainder state.** A Workspace reopened much later could unexpectedly occupy the full sidebar. Only the bounded folded state represents a stable navigation preference; revealing the remainder is a local inspection.
...
- Last updated performs a complete recency sort on entry, then preserves manual adjustments until a user prompt or steer advances one Session and moves it to the front. Returning to Manual preserves every current position. Grouped presentation additionally ranks the Workspace sections by their newest visible Session, which is why Workspace drag handles appear only under Manual order.
...
Domain and Host tests cover durable Workspace moves, no-op and invalid anchors, restart recovery, full-order RPC responses, order frames, and one Workspace snapshot per Host-stream baseline. Runtime tests cover optimistic order, frame/response precedence, overlapping rejection rollback to Host-confirmed order, reconnect baselines, and New Session target priority. Derivation tests cover Workspace sections ranked by their newest visible Session, hidden Sessions not lifting their Workspace, empty Workspaces trailing active ones, and Ungrouped staying last. UI tests cover five-row folding, the blank-row quota and hidden count, collapsed drag anchors across hidden rows, transient expansion reset, search-result reveal beyond the fold, initial and reconnecting Workspace-baseline waits, pruning persisted state after Workspace removal, order-preserving mode switches, one-time recent-update and New Session promotion, Manual drag retention after the first prompt, browser-local Ungrouped and flat-list drag persistence, hierarchy-free flat-row leading spacing, selected view indicators, section re-ranking when a Session updates, drag-handle availability per order mode, expanded-section Workspace hit testing, an unclipped first insertion boundary, outside-list Workspace and Session drops, search collapse rules, and compact CSS dimensions. A shipped-composition Web snapshot pins five established rows beside the provisional New Session.
