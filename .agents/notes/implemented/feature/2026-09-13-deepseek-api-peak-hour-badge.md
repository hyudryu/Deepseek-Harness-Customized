...
The presented windows are the same two windows formatted through `Intl.DateTimeFormat` against `America/Los_Angeles` and against `UTC`, from the billable UTC calendar day containing the instant. Formatting each bound as its own instant is what makes the Pacific line follow daylight saving, so the summer line reads an hour later than the winter one instead of pinning a fixed offset that would be wrong for half the year. The badge asks the reader's own locale for the clock convention and pads the hour only where the locale resolved to 24-hour time, whose `numeric` hour drops the leading zero and would spell the second window `23:00–3:00`.

A weekend instant is drawn from the next billable day instead. No window applies on a UTC Saturday or Sunday, so rendering those hours would name windows that day never has; and the fall-back transition is always a Sunday, where formatting the two bounds across the offset change prints `6:00–9:00 PM` for a Monday whose windows actually run `5:00–8:00 PM`. The tooltip also names the Pacific weekdays beside the times, because the UTC windows open on the previous Pacific day — a reader who plans by `6:00 PM–9:00 PM` alone would take Friday evening for peak and Sunday evening for off-peak, and both are backwards.

### The badge requires both the route and its endpoint

`DEEPSEEK_API_PROVIDER` is the `deepseek-official` id `@deepseek-ai/dsh-llm-deepseek` registers, and `LlmProviderInfo.officialEndpoint` is that adapter's own report that its resolved endpoint is still `PUBLIC_BASE_URL`. Both are required, because the id alone does not establish whose rates apply: `Config.baseURL` and `DEEPSEEK_BASE_URL` redirect that same route, carrying the same model ids, at a proxy or a local server. `DeepSeekAdapter.providerInfo` compares its resolved `baseURL` against `PUBLIC_BASE_URL` and omits the fact when they differ; the fact rides `listProviders()` into each session-controller `ModelProviderGroup`, and `showsPeakSchedule` requires it alongside the id. The id is a wire identity rather than display text, which is why the check does not read the provider's group name.
...
**Gate on the provider id alone.** Rejected: `deepseek-official` describes which adapter owns the route, not which endpoint it talks to, and the adapter's `baseURL` is configurable. A redirected route would then present the DeepSeek platform's windows for requests billed elsewhere, which is the one thing the badge must not do. The endpoint fact costs one optional field on `LlmProviderInfo` — the owner of provider facts — and one projection into the catalog the client already reads.
...
- The badge appears only where the DeepSeek platform's rates apply. A gateway serving DeepSeek models under its own provider id shows nothing, and neither does a `deepseek-official` route redirected through a proxy or a local server, because the adapter reports the endpoint it resolved and the badge requires that report.
...
- One 30-second interval runs per mounted model seat. It is bounded by the number of open sessions, and it disappears with the seat on a route that shows no badge, because the parent renders the badge only for the gated route.
...
Unit: `packages/client/ui-model-selection/tests/peak-hours.client.spec.ts` pins both windows' membership and their closing minutes, the all-weekend rule, the summer and winter Pacific renderings one hour apart, the weekend presentation drawn from the next billable day, the UTC rendering, and the 24-hour locale's padded spelling. `packages/llm/llm-deepseek/tests/adapter.spec.ts` pins both sides of the endpoint fact: the default configuration reports the public API, and a configured `baseURL` stops reporting it.

Component: `packages/client/ui-model-selection/tests/peak-badge.client.spec.tsx` freezes the clock to pin the lit and shaded states, the three-line tooltip in both dictionaries, the fall-back Sunday's tooltip, the keyboard focus that reveals it, and the flip across a window boundary on the badge's own interval. `model-select.client.spec.tsx` pins the scoping rule: the badge rides the gated route, and is absent both for another provider and for a `deepseek-official` group that reports no public endpoint. `packages/api/session-controller/tests/session-models.host.spec.ts` pins the projection, including the group that omits the fact.

Harness: `apps/web/tests/scaffold.ts` reports the replayed DeepSeek route as the public API so the composer goldens keep their badge, and collapses the badge's state line to `{{peak}}` only where it sits directly beneath the model seat, so a transcript line reading `Peak` still has to match its golden.
