...
- The layout owns whether a browser reveal is allowed, and it hands that capability to the browser column at the render site rather than widening the `ctx.layout` service face: `AppFrame` passes `reveal` (its own store action) and the `phone` reading into `renderSlot('browser', …)`, and the new [`revealBrowser`](../../../../packages/client/ui-layout/src/client/stores.ts) action declines while the phone reading is set. The store also reconciles both breakpoint crossings: crossing into the phone layout closes a column an automatic reveal opened while leaving a panel the user opened alone, and ui-browser re-asks for the reveal after leaving the phone layout. The upper-right icon still opens the panel on a phone.
- The [theme presenter](../../../../packages/client/ui-layout/src/client/theme-presenter.ts) republishes the computed base background on the root element beside the `theme-color` metadata it already derives from the same measurement, captures any background the host had already declared so it survives re-application and returns at disposal, and retracts only what it wrote. The color stays the rendered palette's, not a second literal.
- The theme's global [base stylesheet](../../../../packages/client/ui-theme/src/styles/base.css) floors every form control at `max(16px, 1em)` under `:root`-qualified selectors in its phone block, so the guarantee outranks a component's own class selector. The composer already carried that floor for its own card; the search field and address bar kept their 13px and 12px sizes until the theme owned the rule. A larger user font-size preference still wins, and user-initiated pinch zoom stays available: only the automatic focus zoom is removed.
...
**Add the reveal to `ctx.layout`.** A public service verb with one internal consumer asks every layout implementation and test fake to know that consumer's transition; the capability rides the render site instead.
...
The reveal decline is phone-only: wider frames keep revealing a browser the agent opened, and an explicit gesture always opens it. The 16px floor changes the editing size of those compact fields on phones only, which is the deliberate cost of a stable viewport; measured in WebKit at 390px with the theme's content font forced to 14px, every reachable editable — composer, session search, address bar, file input — computes 16px, while a 1280px frame keeps 14px. Pinch zoom remains enabled.

The assembled path is pinned by the keyless [`phone-layout` Web scenario](../../../../apps/web/tests/phone-layout.e2e.ts), which drives the shipped composition at 390x844: every editing surface computes at least 16px there and stays at the user's size at 1280px, the root element carries the body's palette, and the frame reports its browser column collapsed with zero rendered width. Zero model calls are involved, so the scenario runs in replay.
