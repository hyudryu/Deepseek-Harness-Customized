...
For proactive pressure, compaction-basic resolves the route its next request will use — the model selection installed for the Agent scope, else the latest durable request route — resolves that route's adapter capacity and exact-target policy, and scales ratios into a `ResolvedCompactSpec`. It performs this resolution on every check, so a provider or model switch in one session changes capacity and policy immediately, including for the first request under the newly selected model ([route sizing on switch](../bug-fix/2026-09-13-compaction-sizes-the-selected-route.md)). An absolute retained budget that is not below the scaled threshold fails when the target capacity first makes that comparison possible.
