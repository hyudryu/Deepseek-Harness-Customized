---
name: dsh-doc
description: Create, restructure, review, audit, or migrate DeepSeek Harness Markdown documentation, package READMEs, and the documentation website using audience-first hierarchy, kind-mapped YAML metadata, English-only authoring, summary/contents navigation, progressive user-to-developer detail, executed-operation fact-checking, and repository validation. Use for new or revised DSH docs, docs-tree organization, documentation-quality audits and budgets, website page publishing, and documentation structure changes.
---
...
1. Read root and more-specific `AGENTS.md`, [the documentation standard](../../../docs/AGENTS.md), the target page, its source/tests, navigation owner.
...
6. Write and maintain English only. Chinese counterparts, language switchers, and translation sidecars are not required.
...
4. **Check old docs against latest master.** Before revising pre-existing pages, `git fetch origin` and compare the section against `origin/master`. A stale statement on master is still wrong: correct it against the code, not against the old prose.
5. **Validate the English source.** Do not update legacy translations or sidecars as a prerequisite for an English edit.
...
- [Metadata, links, and optional translations](references/metadata-links-i18n.md): README frontmatter, the kind system and its derivation, description semantics, repository paths and legacy translation status.
...
Use [dsh-prose-standard](../dsh-prose-standard/SKILL.md) for sentence-level contract coverage and editorial judgment. The `session-persistence-jsonl` README ([source](../../../packages/session/session-persistence-jsonl/README.md)) is the reference example: searchable YAML, Summary and Table of Contents, user-to-developer progression with a folded developer section, Further Exploration, canonical Model Experience and Known Limitations sections, and a final Dev Note.
...
- English pages: verify structure and links without requiring another language or sidecar.
...
Run `pnpm run test:docs` for the quick comprehensive documentation checks (wrap, links, README gates, budgets, skill metadata, Agent Note gates) before the full `pnpm run doc-sync`.
