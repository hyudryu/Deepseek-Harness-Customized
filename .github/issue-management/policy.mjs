...
export function validateBody({
  ...
  if (count.detailsCount === 0) errors.push('The body must include a collapsed <details> section')
  if (!count.allCollapsed) errors.push('details must be collapsed by default; do not set open')
  if (count.units > BODY_LIMIT) {
    errors.push(`The visible body contains ${count.units} units, exceeding the 50-unit limit`)
  }
  if (normalized.length >= 2 && !owner) {
    errors.push('With multiple assignees, the first nonblank line must be Owner: @login')
    ...
    errors.push('An Owner line is not allowed with zero or one assignee')
  }
  return errors
}
...
export function projectDate(timestamp, timeZone = config.projectTimeZone) {
  ...
  if (Number.isNaN(instant.getTime())) throw new Error(`Invalid PR creation timestamp: ${timestamp}`)
...
export function validateIssue(issue) {
  ...
  if (invalidLabels.length > 0) {
    errors.push(`Issues must not use PR kind or legacy labels: ${invalidLabels.join(', ')}`)
  }
  if (
    ...
    errors.push('Issue titles must not have Type, Priority, Status, area, or Owner prefixes')
  }
  if (!TYPES.has(issue.type ?? '')) errors.push('Type must be Idea, Feature, Bug, Research, or Task')
  if (!status || !config.statuses.includes(status)) errors.push('The Issue must belong to the Project and have a valid Status')
...
export function validatePullRequest(input) {
  ...
  if (input.references.all.length === 0) errors.push('The PR body must reference at least one Issue in the same repository')
  if (kinds.length !== 1) {
    errors.push(`The PR must have exactly one allowed kind/* label; found ${kinds.length}`)
  }
  if (unknownKinds.length > 0) {
    errors.push(`The PR has unsupported kind/* labels: ${unknownKinds.join(', ')}`)
  }
  if (legacyLabels.length > 0) errors.push(`The PR has legacy labels: ${legacyLabels.join(', ')}`)
  if (sourceLabels.length > 0) errors.push(`source/* labels are only allowed on Issues: ${sourceLabels.join(', ')}`)
  if (priorities.length > 1) errors.push(`The PR must have at most one p0-p3 label; found ${priorities.length}`)
  if (areas.length === 0) errors.push('The PR must have at least one area/* label')
  for (const number of input.references.all) {
    if (!input.issues.has(number)) errors.push(`#${number} is not an Issue in the same repository`)
  }
  ...
  if (priorities.length === 0 && issuePriorities.length > 0) {
    ...
    errors.push('A PR with a Priority requires every Issue it resolves to have a Priority')
...
async function projectContext(number, includeStatusActor = false, includeStartDate = false) {
  ...
  if (!project || project.title !== config.projectTitle) throw new Error('The target Project does not exist or its title does not match')
...
async function runPullRequestCheck(event) {
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Issue policy failed with ${errors.length} errors`)
  }
  process.stdout.write(
    requiresPullRequestPolicy(pull) ? 'Issue policy passed.\n' : 'Issue policy enforcement does not yet apply to this PR.\n',
  )
}
