...
- **goal activation** — process-local permission for a continuation consumer to admit another goal round. Activation is either `armed` or `disarmed`; it is deliberately absent from durable replay, so a seeded session (a fork or an inherited child log) requires a later human-authorized resume mutation through `/goal` or the model tool before automatic work. A session reopened as itself instead rearms its own active goal through that same resume mutation, which the continuation driver records at the session start.
