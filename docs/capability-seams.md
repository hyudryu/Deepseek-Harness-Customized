...
flowchart LR
  ...
  svc_sessionMcp["ctx.sessionMcp<br/>Local MCP session supervision"]
  ...
  pkg_session_mcp --> svc_sessionMcp
...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.browserControl` | `core` | [`api-browser-controller`](../packages/api/browser-controller) | - | [`api-browser-controller`](../packages/api/browser-controller) | - | The standalone browser-control plugin owns Playwright contexts; the controller forwards live snapshots and browser operations to the Web client. |
...
| `ctx.usageController` | `core` | [`api-usage-controller`](../packages/api/usage-controller) | - | - | - | Derives historical token usage and completed-turn activity from persisted sessions. |
...
| `ctx.sessionMcp` | `core` | [`session-mcp`](../packages/mcp/session-mcp) | - | - | - | Exposes passive project and session reads plus explicit process-local agent controls over HTTP. |
