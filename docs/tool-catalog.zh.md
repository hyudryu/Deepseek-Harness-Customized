...
加载可用 skill（技能）的完整说明。在执行点名某项 skill 或与其明确匹配的任务前，请使用可用 skill 的确切名称调用此工具。
...
列出可用 skill 类别中的名称和摘要。先在此发现相关 skill，再通过 skill 工具加载完整指令，然后执行任务。
...
来源： [`packages/skill/skill-catalog-buckets/src/index.ts`](../packages/skill/skill-catalog-buckets/src/index.ts)
