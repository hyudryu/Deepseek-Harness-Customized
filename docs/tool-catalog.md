...
| `@deepseek-ai/dsh-super-goal` | `block_super_goal`, `complete_super_goal`, `get_super_goal` | `ctx.tools`, `ctx.agents`, `ctx.commands`, `ctx.userQuestions`, `ctx.sessionProjections` | `tool/call`, `super-goal/change`, `tool/result`, `user/message continuation` | - | Tools are scoped to sessions containing a SuperGoal. Completion requires evidence; hard blockers request two or three human choices before resuming. |
...
| `@deepseek-ai/dsh-skill-catalog-buckets` | `skill_catalog` | `ctx.tools`, `ctx.skills`, `an exact caller-visible skill loader` | `tool/call`, `tool/result` | - | - |
...
Request human input for a hard blocker that prevents further useful progress. Offer 2 to 3 actionable choices; the selected answer resumes pursuit.
...
```json
{
  ...
  "properties": {
    "revision": {
      "type": "number",
      "description": "Exact revision returned by get_super_goal."
    },
    "reason": {
      "type": "string",
      "description": "Concrete blocker requiring human input."
    },
    "choices": {
      "type": "array",
      "description": "Two or three distinct actionable choices.",
      "items": {
        "type": "string"
      }
    }
...
Complete SuperGoal only after verifying the entire objective; provide concrete evidence.
...
```json
{
  ...
  "properties": {
    ...
    "evidence": {
      "type": "string",
      "description": "Observed results proving completion."
    }
  },
  "required": [
    "revision",
...
Read the long-term SuperGoal and exact revision.
...
Tools are scoped to sessions containing a SuperGoal. Completion requires evidence; hard blockers request two or three human choices before resuming.
