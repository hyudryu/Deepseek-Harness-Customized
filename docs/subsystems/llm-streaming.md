...
Retry configuration resolves before route registration into an immutable discriminated union. Normal mode carries `mode: 'normal'`, finite `maxRetries`, `retryableCodes`, `retryDelaysMs`, and required `initialDelayMs`, `maxDelayMs`, and `jitterRatio`; always mode carries `mode: 'always'` and the same required backoff fields without a finite maximum. The two normal-mode delay sources are exclusive: `retryDelaysMs` schedules an explicit wait before each retry and supplies the default `maxRetries` from its length, while `backoff` selects the bounded exponential ramp; configuring both is rejected. Omitting a provider policy uses the normal default of three retries waiting five seconds, one minute, and five minutes, with `maxDelayMs` defaulting to the schedule's longest wait so a provider `Retry-After` the policy could itself schedule is not refused. Layered settings may retain normal-only `maxRetries`, `retryableCodes`, or `retryDelaysMs` after switching to always mode; the resolver ignores those inactive fields and captures the pure always policy. `LlmRuntime.providerRetryPolicy(provider)` returns the registered value, and `llmRetryPolicyOf(stream)` returns the value captured from the serving registration after the call selects it, so later route disposal or replacement cannot change an in-flight failure's recovery policy. The [generated config catalog](../config-catalog.md) lists the optional input fields.
