...
describe('web e2e: plugin configuration section', () => {
  ...
  it('stages SearXNG enablement, rejects invalid URLs, and persists the selected instance', async () => {
    ...
    const dialog = await openPlugins()
    const card = dialog.getByRole('listitem').filter({ has: page.getByText('SearXNG web search', { exact: true }) })
    await card.getByText('SearXNG web search', { exact: true }).click()
    const toggle = card.getByRole('switch', { name: 'Enable SearXNG' })
    const url = card.getByLabel('Instance URL', { exact: true })
    const initialURL = await url.inputValue()
    expect(await toggle.isChecked()).toBe(false)
    await toggle.click()
    await url.fill('not-a-url')
    expect(await card.getByText('Enter an HTTP or HTTPS URL without credentials, a query, or a fragment.', { exact: true }).count()).toBe(1)
    ...
    await card.getByLabel('Request timeout (ms)', { exact: true }).fill('12000')
    await toggle.click()
    ...
    expect(await card.getByLabel('Request timeout (ms)', { exact: true }).inputValue()).toBe('12000')
    await toggle.click()
    await card.getByRole('button', { name: '保存', exact: true }).click()
    await expect.poll(() => card.getByRole('switch').count()).toBe(0)
    await card.getByText('SearXNG web search', { exact: true }).click()
