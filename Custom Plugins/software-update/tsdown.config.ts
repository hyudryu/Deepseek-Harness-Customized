/**
 * Self-contained tsdown config for the software-update custom plugin client
 * bundle. Emits a closure-factory artifact the browser module loader serves a
 * dynamic plugin: `window.__ModuleLoader__.load({id, factory})`, with the
 * loader module-table runtime (React and the shared UI primitives) resolved
 * through the injected `require` and the plugin's own code bundled inline.
 */
import type { UserConfig } from 'tsdown'

const ID = 'dsh-software-update'
