...
3. **After every run, inspect the evidence.** Inspect the returned `screenshot_path` with the `read_image` tool (the `read` tool is text-only and cannot display PNG evidence) and compare the page state against the task's success criteria. Never claim success from `final_result` text alone. If the screenshot shows a mismatch, an error page, a paywall, a consent dialog, or an incomplete flow, send a revised task that names exactly what is wrong and what to do next.
...
- Inspect the returned `screenshot_path` with `read_image`.
