...
`chromeEndpoint` must be an HTTP loopback origin with an explicit port. IPv6 loopback endpoints such as `http://[::1]:9222` retain their address for both the availability probe and CDP connection. `chromeUserDataDir` defaults to `~/.dsh/chrome-profile`; `chromeExecutablePath` optionally names an absolute installed-Chrome executable. `chromeConnectTimeoutMs` defaults to `10000`. `chromeHeadless` defaults to true and launches the dedicated-profile Chrome in the new headless mode at 1280x800 with no taskbar window; set it to false to watch that window. `headless` applies only to Playwright. Chrome credentials and cookies persist. Preexisting tabs, the shared context, and Chrome survive session close and plugin disposal; only session-created tabs and their popups are closed.
...
The panel is interactive: pointer, wheel, and keyboard input over the frame is forwarded to the active page in page CSS pixels, so the user can click, type, scroll, and sign in without a separate window. Input events are validated per kind at the wire boundary before they reach Playwright; a burst of input publishes one coalesced frame refresh. An opening browser reveals the panel even when the agent's `browser` tool started it.
...
Run `node --test "Custom Plugins/browser-control/test/*.test.mjs"`. Tests cover URL/configuration validation, lifecycle concurrency, isolated Playwright behavior, Chrome tab ownership, panel input dispatch, hidden launch flags, and preservation of existing tabs after disposal. Chrome fixtures use temporary profiles and ephemeral ports and close their own processes. Real-provider cases require installed Chrome and Playwright Chromium.
