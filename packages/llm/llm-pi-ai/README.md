...
Each profile may set a `retryPolicy`; omission uses normal mode with three retries waiting five seconds, one minute, and five minutes. A route whose endpoint refuses the connection reports the non-retryable `CONNECTION_REFUSED` rather than `TRANSPORT`, so a stopped local server fails at once instead of after the schedule. `apiKeyEnv` is a credential reference resolved per request through the harness credential seam, so no secret enters the configuration file; a reference that resolves to nothing fails the request with `MISSING_CREDENTIAL`. Omitting it leaves the route configured-but-keyless, which for an installed catalog route defers to pi-ai's provider-native ambient discovery.
