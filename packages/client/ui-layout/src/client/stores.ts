...
import {
  BROWSER_DEFAULT, BROWSER_MAX, BROWSER_MIN, clampWidth, DETAILS_DEFAULT, DETAILS_MAX, DETAILS_MIN,
...
type LayoutActions = {
  ...
  closeBrowser: (draft: LayoutState) => void
}
...
export function createLayoutStore(): EngineStoreHandle<LayoutState, LayoutActions>  {
  const handle = defineStore({
    ...
    actions: {
      ...
      // The browser clamps against the absolute ceiling; the viewport-relative
      // 50% cap is applied by the solver, so a stored preference can exceed it
      // on a wide frame without being rewritten.
      setBrowser: (d, px: number) => { d.browser = clampWidth(px, BROWSER_MIN, BROWSER_MAX) },
