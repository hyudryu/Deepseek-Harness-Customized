---
description: "Web GUI 的外壳布局：四栏 AppFrame、拖动手柄与让步行为、面板几何服务与主题呈现；供窗口外观的用户与维护者阅读。"
...
本包提供 Web GUI 的外壳布局：一个四栏 AppFrame，带可缩放的侧栏、详情与浏览器面板；一条让步链，在空间不足时先收缩并自动关闭详情栏，再处理浏览器栏；以及 `ctx.layout` 面板几何服务，供其他插件调用以打开或关闭详情栏。它还承载主题呈现器，把解析后的配色方案、别名 token、正文字号与 `theme-color` 元数据投影到 document。需要标准窗口外观时选择它；面板几何是瞬时的，重新加载即重置。
...
在 root 槽位挂载本插件；它随即围绕占据侧栏、会话、详情与浏览器栏的内容渲染应用框架。用户拖动不可见命中条带缩放侧栏、拖动浮动胶囊缩放详情面板；窗口变窄时详情栏先收缩并自动关闭，再处理浏览器栏。在桌面宽度下，关闭的侧栏保留 56px 控制栏；详情栏关闭到零宽度。

在宽度不超过 600px 时，会话使用整个框架宽度，不保留常驻轨道。左上角菜单在会话上方打开侧边栏，点击背景遮罩即可关闭。`ctx.layout.closeSidebar()` 幂等地关闭导航，并在窄屏调用时保留桌面的首选宽度。明确打开的详情和浏览器面板在顶部控件下方覆盖会话。移动框架使用紧凑的内容间距和文字，同时保留用户字体偏好、16px 文本输入及浏览器缩放。表格等宽内容保留自身的水平滚动，不会撑宽应用。

右上角的 `browser.toggle` 槽位显示所选会话的浏览器图标。右侧浏览器面板在隐藏时保持挂载；收起操作只改变查看几何状态，不会关闭浏览器上下文。

详情调整手柄跟随详情栏的左边缘，包括浏览器占据最右栏时。
...
- [ui-browser](../ui-browser/README.zh.md)——占据浏览器栏及其右上角切换按钮。
