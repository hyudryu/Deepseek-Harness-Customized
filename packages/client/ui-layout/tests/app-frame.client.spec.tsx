...
describe('AppFrame', () => {
  ...
  it('keeps the details handle on its left edge while the browser is open', () => {
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openDetails(); instance.actions.openBrowser() })
    const [, details, browser] = tracks(frame)
    const handle = frame.querySelector<HTMLElement>('[data-side="details"]')!
    expect(browser).toBeGreaterThan(0)
    expect(handle.style.left).toBe(`${frameWidth - details! - browser!}px`)
    drag(handle, frameWidth - details! - browser!, frameWidth - details! - browser! - 40)
    expect(tracks(frame)[1]).toBe(details! + 40)
    expect(tracks(frame)[2]).toBe(browser)
  })
