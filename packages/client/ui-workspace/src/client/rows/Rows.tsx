...
function sessionStatuses(
  node: Pick<SessionNode, 'pendingInteraction' | 'running' | 'runningSubagentCount' | 'completed' | 'lastTurnFailed'>,
  t: RowTranslate,
  ...
  if (node.lastTurnFailed) {
    const failed: SessionStatus = { state: 'error', label: t('status.failed') }
    return subagents === undefined ? [failed] : [failed, subagents]
  }
