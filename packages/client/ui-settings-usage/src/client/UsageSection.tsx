...
import { activityFor, dailyTotals, modelEncoding, monthMarkers, seriesFor, streaks } from './data.ts'
  ...
  return <div className={css.dashboard} aria-busy={loading}>
    ...
    {summary && <>
      ...
      <section className={css.card} aria-label={t('activity')}>
        ...
        <div className={css.heatScroll}><div className={css.heatmap} aria-hidden="true">{activity.map(cell => <span key={cell.date} className={css.cell} style={{ '--usage-intensity': cell.value === 0 ? 0 : 0.25 + 0.75 * cell.value / activityPeak } as CSSProperties} title={`${cell.date}: ${number(cell.value)} ${t('tokens')}`} />)}</div>
          <div className={css.months}>{monthMarkers(activity.map(cell => cell.date)).map(marker => <span key={marker.date} style={{ gridColumn: marker.column }}>{new Intl.DateTimeFormat(t('language'), { month: 'short', timeZone: 'UTC' }).format(new Date(marker.date))}</span>)}</div></div>
        ...
        <details className={css.tableDetails}><summary>{t('activityData')}</summary><table aria-label={t('activityData')}>
          <caption>{t('activity')}: {t(mode)}</caption><thead><tr><th scope="col">{t('date')}</th><th scope="col">{t('tokens')}</th></tr></thead>
          <tbody>{activity.map(cell => <tr key={cell.date}><th scope="row">{cell.date}</th><td>{new Intl.NumberFormat(t('language')).format(cell.value)}</td></tr>)}</tbody>
        </table></details>
      </section>
      ...
      <section className={css.card} aria-label={t('trend')}><h3>{t('trend')}</h3>
        <div className={css.legend}>{series.models.map(model => <span key={model.key}><svg className={css.swatch} viewBox="0 0 28 8" aria-hidden="true"><line x1="0" y1="4" x2="28" y2="4" stroke={encoding(model.key).color} strokeWidth="2" strokeDasharray={encoding(model.key).dash} /></svg>{modelLabel(model)}</span>)}</div>
        {rangeTotal === 0 ? <p className={css.empty}>{t('noRange')}</p> : <>
          <svg className={css.trend} viewBox="0 0 640 220" role="img" aria-label={t('trend')}>
            ...
            {series.models.map(model => <polyline key={model.key} className={css.line} strokeDasharray={encoding(model.key).dash} style={{ '--usage-color': encoding(model.key).color } as CSSProperties} points={model.values.map((value, day) => `${45 + day * 583 / (range - 1)},${190 - value / peak * 175}`).join(' ')}><title>{modelLabel(model)}: {number(model.total)} {t('tokens')}</title></polyline>)}
      ...
      <section className={css.card} aria-label={t('models')}><h3>{t('models')}</h3><div className={css.breakdown}>
        <div className={css.donutWrap}><svg className={css.donut} viewBox="0 0 200 200" role="img" aria-label={t('models')}><circle className={css.track} cx="100" cy="100" r="76" />{series.models.map((model) => {
          ...
          return <circle key={model.key} className={css.arc} cx="100" cy="100" r="76" pathLength="1" strokeDasharray={`${fraction} ${1 - fraction}`} strokeDashoffset={-offset} style={{ '--usage-color': encoding(model.key).color } as CSSProperties}><title>{modelLabel(model)}: {number(model.total)} {t('tokens')}</title></circle>
        })}</svg><div className={css.donutLabel}><strong>{number(rangeTotal)}</strong><span>{t('tokens')}</span></div></div>
        <ul className={css.modelList}>{series.models.map(model => <li key={model.key}><i style={{ '--usage-color': encoding(model.key).color } as CSSProperties} /><div><strong>{modelLabel(model)}</strong><span>{number(model.total)} {t('tokens')}</span></div><span>{new Intl.NumberFormat(t('language'), { style: 'percent', maximumFractionDigits: 1 }).format(model.total / rangeTotal)}</span></li>)}{rangeTotal === 0 && <li>{t('noRange')}</li>}</ul>
      </div></section>
