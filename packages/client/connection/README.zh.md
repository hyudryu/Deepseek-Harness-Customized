...
`registerListeningAuthority(authority, localAddress)` 仅向接收套接字与该本地接口匹配的请求授予附加主机地址信任。返回的清理函数撤销授权。它不绕过 cookie 或 Origin 检查。[移动访问](../../host/mobile-access/README.zh.md)将授权与 Tailscale 监听器生命周期绑定。
