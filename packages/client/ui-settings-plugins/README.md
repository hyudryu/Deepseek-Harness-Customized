...
Open the Plugins section in Settings and select the **Plugin configuration** tab to edit the host-plane plugins this deployment composes. The cards appear in this order: the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), subagent model selection (`subagent-model-selection`), the DeepSeek search provider (`web-search-deepseek`), and SearXNG (`web-search-searxng`).
...
The SearXNG card stages enablement, the instance URL, and the request timeout. Enabling it selects SearXNG ahead of DeepSeek search without requiring an API key. The instance must serve JSON search responses. URLs accept HTTP(S) and an optional path prefix, without credentials, query parameters, or fragments. Invalid URLs or timeouts block saving; disabling retains the instance settings.
