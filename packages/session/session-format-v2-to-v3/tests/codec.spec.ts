...
const minimalPhysicalHeader = {
  type: 'session', version: 3, id: 'codec', createdAt: 1, isSeeded: false, delegationDepth: 0,
} as const
...
function artifact(
  ...
  return {
    header: { version: 3, id: 'codec', createdAt: 1, isSeeded: false, delegationDepth: 0 },
...
describe('releasedV3SessionFormatCodec headers', () => {
  it('round-trips the minimal and complete optional header images', () => {
    expect(releasedV3SessionFormatCodec.version).toBe(3)
    expect(releasedV3SessionFormatCodec.decodeHeader(minimalPhysicalHeader)).toStrictEqual({
      version: 3, id: 'codec', createdAt: 1, isSeeded: false, delegationDepth: 0,
    })
    expect(releasedV3SessionFormatCodec.decodeHeader(fullPhysicalHeader)).toStrictEqual({
      version: 3,
      id: 'codec',
      createdAt: 1,
      cwd: '/work',
    ...
    const encodedMinimal = releasedV3SessionFormatCodec.encodeArtifact(artifact([]))
    ...
    expect(releasedV3SessionFormatCodec.encodeArtifact(complete).header)
  ...
  it.each([
    ...
    ['wrong type tag', { ...minimalPhysicalHeader, type: 'other' }, /expected released v3/],
    ['wrong version', { ...minimalPhysicalHeader, version: 2 }, /expected released v3/],
    ...
    expect(() => releasedV3SessionFormatCodec.decodeHeader(header)).toThrow(message)
  })
})

describe('releasedV3SessionFormatCodec rows', () => {
  it('stores one event per row and compacts scalar, pair, and long-run provenance exactly', () => {
    ...
    const encoded = releasedV3SessionFormatCodec.encodeArtifact(source)
    ...
    expect(releasedV3SessionFormatCodec.decodeArtifact(encoded.header, encoded.rows)).toStrictEqual(source)
  })
  ...
  it('keeps the v3 physical codec vocabulary-neutral for current growth and a future source freeze', () => {
    const source = artifact([
  ...
  it('expands mixed stored ranges and preserves non-provenance rows', () => {
    ...
    const decoded = releasedV3SessionFormatCodec.decodeArtifact(minimalPhysicalHeader, rows)
    expect(decoded.events[0]).toStrictEqual(feedback(0))
    expect(decoded.events[5]?.sourceEventSeqs).toStrictEqual([0, 1, 2, 4])
    const descending = releasedV3SessionFormatCodec.decodeArtifact(minimalPhysicalHeader, [
  ...
  it.each([
    ...
    expect(() => releasedV3SessionFormatCodec.decodeArtifact(minimalPhysicalHeader, rows)).toThrow(message)
  })
  ...
  it('contains ordinary and non-SessionFormatError row failures in a recoverable tail', () => {
    ...
    expect(releasedV3SessionFormatCodec.decodeRecoverableArtifact(
      minimalPhysicalHeader,
  ...
  it('refuses malformed strict rows, strict gaps, and terminal recoverable tails', () => {
    expect(() => releasedV3SessionFormatCodec.decodeArtifact(minimalPhysicalHeader, [null]))
      .toThrow(/row 0/)
    expect(() => releasedV3SessionFormatCodec.decodeArtifact(minimalPhysicalHeader, [feedback(1)]))
      .toThrow(/seq gap/)
    expect(() => releasedV3SessionFormatCodec.decodeRecoverableArtifact(minimalPhysicalHeader, [{
      type: 'turn/end', seq: 1, time: 1, data: { turn: 1, reason: { kind: 'completed' } },
    }])).toThrow(/seq gap/)
    expect(() => releasedV3SessionFormatCodec.decodeRecoverableArtifact(minimalPhysicalHeader, [null, {
  ...
  it('derives the last tagged seed marker and rejects lineage disagreements', () => {
    ...
    expect(releasedV3SessionFormatCodec.decodeArtifact(seededHeader, markers).inheritedEventCount).toBe(2)
    expect(releasedV3SessionFormatCodec.decodeArtifact(seededHeader, [markers[0]]).inheritedEventCount).toBe(0)
    expect(() => releasedV3SessionFormatCodec.decodeArtifact(seededHeader, []))
      .toThrow(/lacks an inherited end-seed marker/)
    expect(() => releasedV3SessionFormatCodec.decodeArtifact(minimalPhysicalHeader, [markers[0]]))
