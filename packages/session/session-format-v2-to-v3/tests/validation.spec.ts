...
function skillCatalogData(presentationDigest?: string): SessionFormatJsonObject {
  return {
    id: 'catalog',
    role: 'user',
    content: [textBlock],
...
function artifact(
  ...
  return {
    header: { version: 3, id: 'validation', createdAt: 1, isSeeded: false, delegationDepth: 0 },
...
describe('released v3 header validation', () => {
  const minimal = {
    version: 3, id: 'validation', createdAt: 1, isSeeded: false, delegationDepth: 0,
  } as const

  it('accepts minimal and complete logical headers', () => {
    expect(() => { assertReleasedV3Header(minimal) }).not.toThrow()
    expect(() => { assertReleasedV3Header({
      ...minimal,
      cwd: '/work',
  ...
  it.each([
    ...
    ['wrong version', { ...minimal, version: 2 }, /expected format v3/],
    ...
    expect(() => { assertReleasedV3Header(value as SessionFormatHeader) }).toThrow(message)
  })
})

describe('released v3 event envelopes and payloads', () => {
  it('accepts empty artifacts and ordinary log events with a true ignorable marker', () => {
    expect(() => { assertReleasedV3Artifact(artifact([])) }).not.toThrow()
  ...
  it.each([
    ...
    expect(() => { assertReleasedV3Artifact(value) }).toThrow(message)
  })
  ...
  it('validates present and absent opaque payload members before relationship checks', () => {
    for (const includeMeta of [false, true]) {
      ...
      expect(() => { assertReleasedV3Artifact(value) }).toThrow(/no advertised tool lifecycle/)
    }
    const invalidOpaque = {
      ...toolResultData(true),
...
describe('released v3 Assistant streams', () => {
  it('accepts successful, interrupted, legacy-empty, and failed-attempt streams', () => {
    expect(() => { assertReleasedV3Artifact(assistantLifecycle('assistant/message', assistantData())) }).not.toThrow()
    expect(() => { assertReleasedV3Artifact(assistantLifecycle(
      'assistant/message', assistantData({ interrupted: true }),
    )) }).not.toThrow()
    expect(() => { assertReleasedV3Artifact(assistantLifecycle('assistant/message', assistantData({
      content: [], stream: [], usage: null, replayState: null,
    }))) }).not.toThrow()
    expect(() => { assertReleasedV3Artifact(assistantLifecycle('assistant/attempt', {
      turn: 1,
      step: 1,
...
describe('released v3 seed and surface relationships', () => {
  it('accepts tagged seeded lineage and an ordinary unseeded marker', () => {
    expect(() => { assertReleasedV3Artifact(artifact([
      event('session/end-seed', 0, { inherited: true }),
    ], {
      header: { version: 3, id: 'seeded', createdAt: 1, isSeeded: true, delegationDepth: 0 },
      inheritedEventCount: 0,
    })) }).not.toThrow()
    expect(() => { assertReleasedV3Artifact(artifact([
...
describe('released v3 restoration seam', () => {
  it('returns the same validated artifact and preserves safe vocabulary extensions', () => {
    const value = artifact([])
    expect(restoreReleasedV3Artifact(value, new Set(RELEASED_V3_EVENT_TYPES))).toBe(value)
    const ignorableExtension = artifact([
      event('external/ignorable', 0, { retained: true }, { ignorable: true }),
    ])
    expect(restoreReleasedV3Artifact(ignorableExtension, new Set(RELEASED_V3_EVENT_TYPES)))
    ...
    expect(restoreReleasedV3Artifact(
      installedExtension,
      new Set([...RELEASED_V3_EVENT_TYPES, 'external/installed']),
    )).toBe(installedExtension)
    const extendedPayload = artifact([event('turn/start', 0, { turn: 1, postReleaseMember: true })])
    expect(restoreReleasedV3Artifact(extendedPayload, new Set(RELEASED_V3_EVENT_TYPES)))
      .toBe(extendedPayload)
    expect(() => restoreReleasedV3Artifact(
      artifact([event('external/unknown', 0, {})]),
      new Set(RELEASED_V3_EVENT_TYPES),
