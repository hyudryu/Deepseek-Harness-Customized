...
import { RELEASED_V3_EVENT_DISPOSITIONS, RELEASED_V3_EVENT_TYPES } from './dispositions.ts'
...
const RELEASED_V3_EVENT_TYPE_SET = new Set(RELEASED_V3_EVENT_TYPES)
const RELEASED_V3_RELATIONSHIP_EXTENSIONS = {
  stepEvents: new Set(['assistant/attempt']),
  preservedSourceTitleRequestText: true,
} as const

/**
 * Validate the exact logical header written by released v3.
 * @param header - decoded released-v3 Session header.
 * @throws {SessionFormatError} when the header is not an exact released-v3 value.
 */
export function assertReleasedV3Header(header: SessionFormatHeader): void {
  const record = jsonRecord(header, 'format v3 header')
  exactKeys(record, HEADER_REQUIRED, HEADER_OPTIONAL, 'format v3 header')
  if (record['version'] !== 3) throw new SessionFormatError('expected format v3 header')
  if (typeof record['id'] !== 'string') throw new SessionFormatError('format v3 header id must be a string')
  sessionFormatCount(record['createdAt'], 'format v3 header createdAt')
  sessionFormatCount(record['delegationDepth'], 'format v3 header delegationDepth')
  if (typeof record['isSeeded'] !== 'boolean') throw new SessionFormatError('format v3 header isSeeded must be boolean')
  if (record['cwd'] !== undefined && (typeof record['cwd'] !== 'string' || !isAbsolute(record['cwd']))) {
    throw new SessionFormatError('format v3 header cwd must be absolute')
  }
  for (const key of ['parentSession', 'agentPreset'] as const) {
    if (record[key] !== undefined && typeof record[key] !== 'string') {
      throw new SessionFormatError(`format v3 header ${key} must be a string`)
    }
  }
  if (record['origin'] !== undefined && record['origin'] !== 'subagent') {
    throw new SessionFormatError('format v3 header origin must be "subagent"')
  }
}

/**
 * Validate the exact logical image emitted by the released v3 writer.
 * @param artifact - complete decoded released-v3 Session artifact.
 ...
 */
export function assertReleasedV3Artifact(artifact: SessionFormatArtifact): void {
  validateReleasedV3Artifact(artifact, 'target', RELEASED_V3_EVENT_TYPE_SET)
}

/**
 * Validate only the released-v3 physical header, event envelopes, and inherited cut.
 ...
 */
export function assertReleasedV3PhysicalArtifact(artifact: SessionFormatArtifact): void {
  validateReleasedV3Artifact(artifact, 'physical')
}

function validateReleasedV3Artifact(
  ...
  const cut = sessionFormatCount(artifact.inheritedEventCount, 'format v3 inherited event count')
  if (cut > artifact.events.length) throw new SessionFormatError('format v3 inherited event count exceeds its events')
  if (!artifact.header.isSeeded && cut !== 0) throw new SessionFormatError('unseeded format v3 Session has inherited events')
  let lastInheritedMarker: number | undefined
  for (const [index, event] of artifact.events.entries()) {
    const record = jsonRecord(event, `format v3 event ${index}`)
    const type = record['type']
    if (typeof type !== 'string') throw new SessionFormatError(`format v3 event ${index} type must be a string`)
    const disposition = RELEASED_V3_EVENT_DISPOSITIONS[type]
    ...
    if (mode !== 'physical' && disposition === undefined && !installed && !ignorableUnknown) {
      throw new SessionFormatUnsupportedMigrationError(
        `format v3 contains unknown event type ${JSON.stringify(type)} at seq ${index}`,
      )
    }
    ...
    exactKeys(record, EVENT_REQUIRED, optional, `format v3 event ${index}`)
    if (record['seq'] !== index) throw new SessionFormatError(`format v3 event ${index} is not dense`)
    sessionFormatSafeInteger(record['time'], `format v3 event ${index} time`)
    if (record['ignorable'] !== undefined && record['ignorable'] !== true) {
      throw new SessionFormatError(`format v3 event ${index} ignorable must be true when present`)
    }
  ...
  if (artifact.header.isSeeded && lastInheritedMarker !== cut) {
    throw new SessionFormatError('format v3 seeded header disagrees with its last inherited end-seed marker')
  }
  ...
  if (mode === 'target') {
    assertReleasedArtifactRelationships(artifact, RELEASED_V3_RELATIONSHIP_EXTENSIONS)
  }
}

function assertPayload(
  event: SessionFormatEvent,
  disposition: (typeof RELEASED_V3_EVENT_DISPOSITIONS)[string],
): void {
  ...
  if (event.type === 'assistant/attempt' || event.type === 'assistant/message') {
    ...
    assertReleasedPayloadSemantics(event, 3)
    if (timed.length > 0) {
...
/**
 * Restore and validate one decoded released-v3 artifact.
...
export function restoreReleasedV3Artifact(
  ...
  validateReleasedV3Artifact(artifact, 'current', knownEventTypes)
  return artifact
}
