...
`listenOn(address, accepts)` 在主端口添加绑定指定接口的监听器，在通过相同 HTTP 和升级路由分发前执行提供的请求策略。其异步清理函数仅关闭该监听器及其套接字。调用方负责策略，必须保留并调用清理函数，[移动访问插件](../mobile-access/README.zh.md)采用此方式。
